...
"""
    ...
    Sist publiserte versjon hentes fra nbvok ved 'https://api.nb.no/nbvok/rest/v1/<graphname>/data'
...
def konverter(filnavn, sistpublisert, konfignavn, konfigsti, utfilsti):
    ...
    #sistpublisert er grafnavnet på vokabularet i nbvok, f.eks. 'neo', 'ntsf'
    sys.path.append(konfigsti)
    ...
    print ("\n\nResultat-fil: ", utfilsti+utfilnavn)
    new_gr.serialize(destination=utfilsti+utfilnavn, format='ttl', encoding='utf-8')
...
def fiksRDF(dom, konf):
    ...
    for concept in dom.findall('/skos:Concept', ns_map):
        ...
        for matchnode in concept.findall('skos:relatedMatch', ns_map):    
            concept2 = matchnode.find("skos:Concept", ns_map)
            if concept2 is not None:
                about = concept2.get(rdf+"about")
                if not konf.tematres_uribase in about:
                    ny = etree.Element(skos+"relatedMatch")
                    ny.attrib[rdf+"resource"] = about
                    concept.remove(matchnode)
                    concept.append(ny)
